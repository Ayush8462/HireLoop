...
export class ReferralService {
    async requestReferral(authUserId, data) {
        ...
        const existing = await referralRepository.findExistingPending(studentProfile._id, seniorObjectId, companyObjectId, data.jobTitle);
        if (existing) {
    ...
    async updateStatus(authUserId, referralId, status) {
    ...
    async getMySentReferrals(authUserId, status) {
        const studentProfile = await profileRepository.findByAuthUserId(authUserId);
        if (!studentProfile) {
    ...
    async getMyReceivedReferrals(authUserId, status) {
